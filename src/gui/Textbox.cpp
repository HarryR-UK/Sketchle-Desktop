...
// this function has the timer i.e. for making the cursor blink off and on
void Textbox::update(const Input& input, float dt, float elapsed){
    sf::Vector2f boxPos = mTboxShape.getPosition();
    ...
    if(mousePos->x < boxWidth && mousePos->x > boxPos.x && mousePos->y < boxHeight && mousePos->y > boxPos.y){
        ...
        if(input.mouseButton1Clicked){
            mIsSelected = true;
            mShowCursor = true; // reset blink state and timer so cursor visible
            mCursorTimer = 0.f;
            if(mUserInput.empty()){
                mText.setString("_"); // clear input if showing placeholder text
            } else {
    ...
    if(mIsSelected){
        ...
        // make the cursor blink on a timer
        mCursorTimer += dt;
        if(mCursorTimer >= 0.5f){
